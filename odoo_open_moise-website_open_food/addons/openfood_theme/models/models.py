...
class DeliveryTimetable(models.Model):
    ...
    delivery_man = fields.Many2one(comodel_name='res.partner', string='Livreur', domain="[('is_driver', '=', True)]")
    date = fields.Date(string='Date du check')
    checked_time = fields.Many2many(comodel_name='delivery.available_time', string='Heure sélectionnée')
